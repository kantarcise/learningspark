...
/**
 * Let's understand how Windows work in Spark!
 */
object SimpleWindowing {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("groupBy region, salesperson, with total_sales\n")
    totalSales.show()
    ...
    println("Here is a more general comparison of window functions\n")

    val randomDF = spark.createDataFrame(Seq(
      ("a", 10), ("a", 20), ("a", 30),
      ("a", 40), ("a", 40), ("a", 40), ("a", 40),
      ("a", 50), ("a", 50), ("a", 60)
    )).toDF("part_name", "order_count")

    val window = Window
      // all the same parts = a
      .partitionBy("part_name")
